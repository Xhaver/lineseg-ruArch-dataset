...
Dataset/
├── sources/
│   ├── full_annotated/              # Original annotated DXF files (source of truth)
│   │   └── 1_full_annotated.dxf
│   ├── unprocessed_geometry_only/   # After extract_geometry.py (geometry extracted)
│   │   └── 1_geometry_only.dxf
│   ├── geometry_only/               # After preprocess_dxf.py (LINE-only, ready to crop)
│   │   └── 1_geometry_only.dxf
│   └── dwg/                         # Original DWG source files
│
├── drawings/
│   └── 001/                         # One folder per drawing (zero-padded ID)
...
├── yolo_dataset/                    # YOLO training data (generated by prepare_yolo_dataset.py)
└── scripts/
    ├── run.py                       # Interactive menu (entry point)
    ├── extract_geometry.py          # Step 0: extract geometry from full_annotated/
    ├── preprocess_dxf.py            # Step 1: convert all primitives to LINE segments
    ├── make_crops.py                # Step 2: rasterize and crop
    ├── add_noise.py                 # Step 3: generate noisy variants
    ├── prepare_yolo_dataset.py      # Generate YOLO dataset from full_annotated/
    ├── train_yolo.py                # Train YOLO model
    ├── eval_yolo.py                 # Evaluate YOLO model
    ├── make_preview.py              # Generate noise preview image
    ├── paths.py                     # Shared path constants
    └── loader.py                    # Dataset loader utilities
```
...
Per drawing: N crops (variable, depends on drawing size and crop settings)
× (1 clean + 20 noisy variants) = N × 21 samples.
...
> The rasterized dataset is generated locally and is not committed to git.
> To build the full dataset, run the pipeline for all sources (menu option **6**):
...
| # | Option |
|---|---|
| 1 | Extract geometry — `full_annotated/` → `unprocessed_geometry_only/` |
| 2 | Preprocess DXF — `unprocessed_geometry_only/` → `geometry_only/` |
| 3 | Rasterize & crop one drawing — `geometry_only/` → `drawings/` |
| 4 | Generate noisy variants for one drawing |
| 5 | Full pipeline (steps 2–4) for one drawing |
| 6 | Full pipeline (steps 2–4) for ALL drawings |
| 7 | Dataset statistics |
| 8 | Generate YOLO dataset — `full_annotated/` → `yolo_dataset/` |
| 9 | Train YOLO model |
| 10 | Evaluate YOLO model (metrics + visualization) |
| 11 | Delete all generated data (`drawings/` and `manifest.json`) |
| q | Quit |

---

#### `extract_geometry.py` — extract geometry from annotated DXF

Reads `*_full_annotated.dxf` from `sources/full_annotated/` and writes
`*_geometry_only.dxf` to `sources/unprocessed_geometry_only/`.

Removes all non-geometry entities (TEXT, MTEXT, DIMENSION, LEADER, MULTILEADER,
HATCH, POINT, VIEWPORT, ATTDEF, ATTRIB) and everything on annotation layers
(`YOLO_TABLE`, `YOLO_STAMP`). Retains LINE, LWPOLYLINE, ARC, CIRCLE, ELLIPSE,
SPLINE, INSERT — curved entities are preserved here for subsequent filtering
by `preprocess_dxf.py`.

```bash
# Process all *_full_annotated.dxf in sources/full_annotated/ (default)
...
Reads `*_geometry_only.dxf` from `sources/unprocessed_geometry_only/`,
converts all geometry to individual LINE entities, and writes the result to
`sources/geometry_only/`.

By default (`--skip-curved` is on), ARC, CIRCLE, and ELLIPSE entities are
**dropped** (recommended: removes door-swing arcs and other circular annotations
from ground truth). Pass `--no-skip-curved` to convert them to chord
approximations instead.

```bash
# Process all files in unprocessed_geometry_only/ (default)
...
# Keep curved entities (convert ARC/CIRCLE/ELLIPSE to chord segments)
python scripts/preprocess_dxf.py --units m --no-skip-curved
```

| Argument | Default | Description |
|---|---|---|
| `path` | `unprocessed_geometry_only/` | File or folder to process |
...
| `--skip-curved` | on | Drop ARC/CIRCLE/ELLIPSE instead of approximating them |
| `--no-skip-curved` | — | Convert ARC/CIRCLE/ELLIPSE to chord LINE segments |
...
python scripts/make_crops.py sources/geometry_only/1_geometry_only.dxf --scale 100 --units m --dry-run

# Rasterize and crop
python scripts/make_crops.py sources/geometry_only/1_geometry_only.dxf --scale 100 --units m
```
...
The global manifest is a JSON object with three top-level keys:
...
The full pipeline has four steps. Steps 1 and 2 operate on the `sources/` subdirectories;
steps 3 and 4 operate on the generated `drawings/` tree.

```bash
# Step 0: place the annotated source
#   sources/full_annotated/N_full_annotated.dxf

# Step 1: extract geometry (removes text, dimensions, annotation layers)
python scripts/extract_geometry.py sources/full_annotated/N_full_annotated.dxf

# Step 2: preprocess (convert LWPOLYLINE → LINE, drop/approximate curved entities)
python scripts/preprocess_dxf.py sources/unprocessed_geometry_only/N_geometry_only.dxf --units m

# Step 3: rasterize and crop
python scripts/make_crops.py sources/geometry_only/N_geometry_only.dxf --scale 100 --units m
...
Or use the interactive menu (options 5 or 6) to run all steps at once.

---

### Preparing source DXF files

Two types of DXF files are used. Both reside in `sources/` subdirectories and must be
**prepared in CAD before running the scripts**.

---

#### `*_full_annotated.dxf` — complete drawing with annotations

This is the primary source file placed in `sources/full_annotated/`.
`extract_geometry.py` reads it to produce the geometry-only DXF.
`prepare_yolo_dataset.py` reads it to produce YOLO training labels.

The file contains the full drawing — all geometry, text, dimensions, and manual
annotation markers (YOLO_TABLE, YOLO_STAMP polylines).
...
5. Save as `N_full_annotated.dxf` in `sources/full_annotated/`.

---

#### `*_geometry_only.dxf` — vectorization ground truth (auto-generated)

This file is **produced automatically** by `extract_geometry.py` from the corresponding
`*_full_annotated.dxf`. You do not create it manually.

`extract_geometry.py` removes TEXT, MTEXT, DIMENSION, LEADER, HATCH, VIEWPORT,
and all entities on `YOLO_TABLE` / `YOLO_STAMP` layers. The remaining geometry
(LINE, LWPOLYLINE, ARC, CIRCLE, ELLIPSE, INSERT) is saved to
`sources/unprocessed_geometry_only/`.

`preprocess_dxf.py` then converts LWPOLYLINE to individual LINE segments, handles
INSERT explosion, filters coordinate outliers, and writes the final LINE-only DXF
to `sources/geometry_only/`.

**What ends up in the ground truth:**

| Element | Outcome | Notes |
|---|---|---|
| Walls, slabs, beams | ✅ Kept | Core structural geometry |
| Doors, windows (geometry) | ✅ Kept | Extracted automatically |
| Door-swing arcs | ❌ Dropped | Removed by `--skip-curved` in preprocess_dxf.py |
| Round columns (CIRCLE) | ❌ Dropped | Removed by `--skip-curved` |
| Circular staircases | ❌ Dropped | Removed by `--skip-curved` |
| Sanitary equipment, furniture | ✅ Kept if present | Remove manually in CAD if it produces noisy GT |
| Room-number circles | ❌ Dropped | Removed by `--skip-curved` |
| Text, dimensions | ❌ Dropped | Removed by extract_geometry.py |
| Title block / stamp | ❌ Dropped | Removed by extract_geometry.py (YOLO_STAMP layer) |
| Sheet frame | ❌ Dropped | Remove manually in CAD — `YOLO_FRAME` layer is NOT excluded by extract_geometry.py |

> **Tip:** to keep curved geometry in GT (e.g. circular columns), pass `--no-skip-curved`
> to `preprocess_dxf.py`. Arcs will be converted to chord approximations.

**If manual corrections are needed** (e.g. to remove furniture, sanitary equipment,
or sheet frames that weren't annotated with `YOLO_FRAME`), edit the files in
`sources/unprocessed_geometry_only/` directly in CAD, then re-run `preprocess_dxf.py`.
...
Dataset/
├── sources/
│   ├── full_annotated/              # Исходные аннотированные DXF (источник истины)
│   │   └── 1_full_annotated.dxf
│   ├── unprocessed_geometry_only/   # После extract_geometry.py (геометрия без аннотаций)
│   │   └── 1_geometry_only.dxf
│   ├── geometry_only/               # После preprocess_dxf.py (только LINE, готово к нарезке)
│   │   └── 1_geometry_only.dxf
...
├── yolo_dataset/                    # Обучающие данные YOLO (генерируется prepare_yolo_dataset.py)
└── scripts/
    ├── run.py                       # Интерактивное меню (точка входа)
    ├── extract_geometry.py          # Шаг 0: извлечение геометрии из full_annotated/
    ├── preprocess_dxf.py            # Шаг 1: приведение к LINE-сегментам
    ├── make_crops.py                # Шаг 2: растеризация и нарезка
    ├── add_noise.py                 # Шаг 3: генерация шума
    ├── prepare_yolo_dataset.py      # Генерация YOLO-датасета из full_annotated/
    ├── train_yolo.py                # Обучение YOLO-модели
    ├── eval_yolo.py                 # Оценка YOLO-модели
    ├── make_preview.py              # Генерация превью шумов
    ├── paths.py                     # Общие константы путей
    └── loader.py                    # Утилиты загрузки датасета
```
...
На каждый чертёж: N кропов (зависит от размера чертежа и параметров нарезки)
× (1 чистый + 20 зашумлённых вариантов) = N × 21 сэмплов.
...
> Растеризованный датасет генерируется локально и не хранится в git.
> Для сборки полного датасета запустите пайплайн для всех исходников (пункт меню **6**):
...
| # | Действие |
|---|---|
| 1 | Извлечь геометрию — `full_annotated/` → `unprocessed_geometry_only/` |
| 2 | Препроцессировать DXF — `unprocessed_geometry_only/` → `geometry_only/` |
| 3 | Нарезать чертёж на кропы — `geometry_only/` → `drawings/` |
| 4 | Сгенерировать шумовые варианты для одного чертежа |
| 5 | Полный пайплайн (шаги 2–4) для одного чертежа |
| 6 | Полный пайплайн (шаги 2–4) для ВСЕХ чертежей |
| 7 | Статистика датасета |
| 8 | Сгенерировать YOLO-датасет — `full_annotated/` → `yolo_dataset/` |
| 9 | Обучить YOLO-модель |
| 10 | Оценить YOLO-модель (метрики + визуализация) |
| 11 | Очистить сгенерированные данные (`drawings/` и `manifest.json`) |
| q | Выход |

---

#### `extract_geometry.py` — извлечение геометрии из аннотированного DXF

Читает `*_full_annotated.dxf` из `sources/full_annotated/` и записывает
`*_geometry_only.dxf` в `sources/unprocessed_geometry_only/`.

Удаляет все не-геометрические сущности (TEXT, MTEXT, DIMENSION, LEADER, MULTILEADER,
HATCH, POINT, VIEWPORT, ATTDEF, ATTRIB) и всё на слоях `YOLO_TABLE`, `YOLO_STAMP`.
Сохраняет LINE, LWPOLYLINE, ARC, CIRCLE, ELLIPSE, SPLINE, INSERT — криволинейные
сущности сохраняются для последующей фильтрации в `preprocess_dxf.py`.

```bash
# Все файлы из sources/full_annotated/ (по умолчанию)
python scripts/extract_geometry.py
...
Читает `*_geometry_only.dxf` из `sources/unprocessed_geometry_only/`,
конвертирует всю геометрию в LINE-сущности и записывает результат в
`sources/geometry_only/`.

По умолчанию (`--skip-curved` включён) ARC, CIRCLE и ELLIPSE **удаляются**
(рекомендуется: убирает дуги качания дверей и другие круговые аннотации из GT).
Передайте `--no-skip-curved`, чтобы конвертировать их в хордовые аппроксимации.

```bash
# Все файлы в unprocessed_geometry_only/ (по умолчанию)
python scripts/preprocess_dxf.py --units m
...
# Сохранить кривые (конвертировать ARC/CIRCLE/ELLIPSE в хорды)
python scripts/preprocess_dxf.py --units m --no-skip-curved
```

| Аргумент | По умолчанию | Описание |
|---|---|---|
| `path` | `unprocessed_geometry_only/` | Файл или папка для обработки |
...
| `--skip-curved` | вкл. | Удалять ARC/CIRCLE/ELLIPSE вместо аппроксимации |
| `--no-skip-curved` | — | Конвертировать ARC/CIRCLE/ELLIPSE в хордовые LINE-сегменты |
...
Глобальный манифест — JSON-объект с тремя ключами верхнего уровня:
...
Пайплайн состоит из четырёх шагов. Шаги 0–2 работают с подкаталогами `sources/`;
шаги 3–4 — с деревом `drawings/`.

```bash
# Шаг 0: разместить аннотированный исходник
#   sources/full_annotated/N_full_annotated.dxf

# Шаг 1: извлечь геометрию (убрать текст, размеры, аннотационные слои)
python scripts/extract_geometry.py sources/full_annotated/N_full_annotated.dxf

# Шаг 2: препроцессировать (LWPOLYLINE → LINE, убрать/аппроксимировать дуги)
python scripts/preprocess_dxf.py sources/unprocessed_geometry_only/N_geometry_only.dxf --units m

# Шаг 3: растеризовать и нарезать на кропы
python scripts/make_crops.py sources/geometry_only/N_geometry_only.dxf --scale 100 --units m

# Шаг 4: сгенерировать шум
python scripts/add_noise.py drawings/00N
```

Чертёж автоматически получит ID `00N`, его сэмплы будут добавлены в `manifest.json`.
Или используйте интерактивное меню (пункты 5 или 6) для запуска всех шагов сразу.

---

### Подготовка исходных DXF-файлов

Используются два типа DXF-файлов, оба хранятся в подкаталогах `sources/`.
Оба **готовятся в CAD до запуска скриптов**.

---

#### `*_full_annotated.dxf` — полный чертёж с аннотациями

Основной исходный файл, размещается в `sources/full_annotated/`.
`extract_geometry.py` читает его для создания геометрии-эталона.
`prepare_yolo_dataset.py` читает его для создания YOLO-меток.

Файл содержит полный чертёж — всю геометрию, текст, размеры и ручные
маркеры аннотационных зон (полилинии YOLO_TABLE, YOLO_STAMP).
...
5. Сохранить как `N_full_annotated.dxf` в `sources/full_annotated/`.

---

#### `*_geometry_only.dxf` — эталон для векторизации (генерируется автоматически)

Этот файл **создаётся автоматически** скриптом `extract_geometry.py` из
соответствующего `*_full_annotated.dxf`. Вручную его готовить не нужно.

`extract_geometry.py` удаляет TEXT, MTEXT, DIMENSION, LEADER, HATCH, VIEWPORT
и все сущности на слоях `YOLO_TABLE` / `YOLO_STAMP`. Оставшаяся геометрия
(LINE, LWPOLYLINE, ARC, CIRCLE, ELLIPSE, INSERT) сохраняется в
`sources/unprocessed_geometry_only/`.

`preprocess_dxf.py` конвертирует LWPOLYLINE в отдельные LINE-сегменты,
обрабатывает INSERT-блоки, фильтрует выбросы координат и записывает
итоговый LINE-only DXF в `sources/geometry_only/`.

**Что попадает в GT:**

| Элемент | Результат | Примечание |
|---|---|---|
| Стены, плиты, балки | ✅ Сохраняется | Основная геометрия |
| Двери, окна (геометрия) | ✅ Сохраняется | Извлекается автоматически |
| Дуги качания дверей | ❌ Удаляется | Убирает `--skip-curved` в preprocess_dxf.py |
| Круглые колонны (CIRCLE) | ❌ Удаляется | Убирает `--skip-curved` |
| Круговые лестничные клетки | ❌ Удаляется | Убирает `--skip-curved` |
| Сантехника, мебель | ✅ Сохраняется если есть | Удалить вручную в CAD, если GT получается шумным |
| Кружки номеров помещений | ❌ Удаляется | Убирает `--skip-curved` |
| Текст, размеры | ❌ Удаляется | Убирает extract_geometry.py |
| Штамп / основная надпись | ❌ Удаляется | Убирает extract_geometry.py (слой YOLO_STAMP) |
| Рамка листа | ❌ Удалить вручную в CAD | Слой `YOLO_FRAME` extract_geometry.py **не** исключает |

> **Совет:** чтобы сохранить криволинейную геометрию в GT (например, круглые колонны),
> передайте `--no-skip-curved` в `preprocess_dxf.py`. Дуги будут преобразованы в хорды.

**Если нужны ручные правки** (например, убрать мебель, сантехнику или рамку листа,
которые не были размечены слоем `YOLO_FRAME`), отредактируйте файлы в
`sources/unprocessed_geometry_only/` в CAD, а затем повторно запустите `preprocess_dxf.py`.
